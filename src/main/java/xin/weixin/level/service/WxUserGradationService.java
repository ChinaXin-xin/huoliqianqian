package xin.weixin.level.service;

import lombok.Data;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import xin.weixin.level.domain.WxUserGradation;
import xin.weixin.level.mapper.WxUserGradationMapper;
...
@Service
@Data
public class WxUserGradationService {

    @Autowired
    private WxUserGradationMapper userGradationMapper;

    // 用于存储用户信息的内存树结构
    private Map<Integer, WxUserGradation> userMap = new HashMap<>();

    // 在服务启动时，从数据库加载所有用户信息
    @PostConstruct
    public void init() {
        List<WxUserGradation> users = userGradationMapper.selectList(null);
    ...
    public List<String> getUserAndDescendants(String userName) {
        return userMap.values().stream()
                ...
                .map(WxUserGradation::getUserName)
    ...
    public void addChild(String parentUserName, String childUserName) {
        WxUserGradation parentUser = userMap.values().stream()
                .filter(user -> user.getUserName().equals(parentUserName))
                .findFirst()
                .orElse(null);

        if (parentUser != null) {
            WxUserGradation child = new WxUserGradation();
    ...
    // 辅助方法，检查是否为子节点
    private boolean isDescendant(WxUserGradation user, String parentUserName) {
        Integer parentId = user.getParentId();
        while (parentId != null) {
            WxUserGradation parent = userMap.get(parentId);
    ...
    public List<String> getUsersAtSameLevel(String userName) {
        WxUserGradation targetUser = findUserByName(userName);
        if (targetUser == null) {
    ...
    private WxUserGradation findUserByName(String userName) {
        return userMap.values().stream()
                .filter(user -> user.getUserName().equals(userName))
                .findFirst()
                .orElse(null);
    ...
    // 辅助方法：获取用户在树中的层级
    private int getLevel(WxUserGradation user) {
        int level = 0;
    ...
    public int getUserLevel(String userName) {
        WxUserGradation user = findUserByName(userName);
        if (user == null) {
    ...
    public boolean isDescendant(String userName1, String userName2) {
        WxUserGradation user1 = findUserByName(userName1);
        WxUserGradation user2 = findUserByName(userName2);
    ...
    // 辅助递归方法，检查是否为子节点
    private boolean isDescendantHelper(Integer parentId, WxUserGradation child) {
    ...
    public boolean isAncestor(String childrenUser, String parentUser) {
        WxUserGradation user1 = findUserByName(childrenUser);
        WxUserGradation user2 = findUserByName(parentUser);
    ...
    public List<String> getDirectDescendants(String userName) {
        WxUserGradation parentUser = findUserByName(userName);
        if (parentUser == null) {
            return new ArrayList<>(); // 如果用户不存在，返回空列表
        }
    ...
    public String getDirectParent(String userName) {
        ...
        WxUserGradation parentUser = userMap.get(user.getParentId());
