...
import xin.zhongFu.constants.ByteConstants;
...
public class ByteUtil {
    ...
    public static byte[] getBytes(byte[] data, byte oldFlag, byte newFlag) {
    ...
    public static byte[] hexStringToByte(String hex) {
        ...
        for (int j = 0; j < i; j += ByteConstants.BCD_LENGTH) {
    ...
    public static byte[] str2Bcd(String asc) {
        ...
        if (len >= ByteConstants.BCD_LENGTH) {
            len /= 2;
        }
        ...
        for (int p = 0; p < asc.length() / ByteConstants.BCD_LENGTH; p++) {
            int j;
